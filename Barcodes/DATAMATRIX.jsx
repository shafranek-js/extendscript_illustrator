...
var readIni = File("~/Desktop/DataMatrix.ini");    
    if (readIni.exists){ 
    readIni.open("r");
...
var str = arr[0];
var userInputSize = arr[1];

if (1==1){ //just to fold long dialog code
// DIALOG
// ======
var dialog = new Window("dialog"); 
    dialog.text = "DATAMATRIX"; 
...
// COMMONPANEL
// ===========
var CommonPanel = dialog.add("panel", undefined, undefined, {name: "CommonPanel"}); 
    CommonPanel.orientation = "row"; 
    CommonPanel.alignChildren = ["center","top"]; 
    CommonPanel.spacing = 1; 
    CommonPanel.margins = 15; 
    CommonPanel.alignment = ["left","top"]; 

// FIELDSGROUP
// ===========
var FieldsGroup = CommonPanel.add("group", undefined, {name: "FieldsGroup"}); 
    FieldsGroup.orientation = "column"; 
    FieldsGroup.alignChildren = ["right","center"]; 
    FieldsGroup.spacing = 2; 
    FieldsGroup.margins = 0; 

// CODE
// ====
var Code = FieldsGroup.add("group", undefined, {name: "Code"}); 
    Code.orientation = "row"; 
    Code.alignChildren = ["left","center"]; 
    Code.spacing = 10; 
    Code.margins = 0; 

var CodeStaticText = Code.add("statictext", undefined, undefined, {name: "CodeStaticText"}); 
    CodeStaticText.text = "Code:"; 

var CodeValue = Code.add('edittext {justify: "right", properties: {name: "CodeValue"}}'); 
    CodeValue.text = str; 
    CodeValue.preferredSize.width = 82; 

// SIZE
// ====
var Size = FieldsGroup.add("group", undefined, {name: "Size"}); 
    Size.orientation = "row"; 
    Size.alignChildren = ["left","center"]; 
    Size.spacing = 10; 
    Size.margins = 0; 

var SizeStaticText = Size.add("statictext", undefined, undefined, {name: "SizeStaticText"}); 
    SizeStaticText.text = "Size (mm):"; 

var SizeValue = Size.add('edittext {justify: "right", properties: {name: "SizeValue"}}'); 
    SizeValue.text =userInputSize ; 
    SizeValue.preferredSize.width = 51; 
...
// BUTTONPANEL
// ===========
var ButtonPanel = ButtonGroup.add("panel", undefined, undefined, {name: "ButtonPanel"}); 
    ButtonPanel.orientation = "column"; 
    ButtonPanel.alignChildren = ["fill","center"]; 
    ButtonPanel.spacing = 10; 
    ButtonPanel.margins = 7; 

var ok = ButtonPanel.add("button", undefined, undefined, {name: "ok"}); 
    ok.text = "OK"; 
    ok.onClick = main;
...
//сохраняем ini файл
function saveIni() {
    var file = new File('~/Desktop/DataMatrix.ini');   
    file.open("w");
    file.writeln( CodeValue.text);
    file.writeln( SizeValue.text );
    file.close();
    }
...
function newDoc() {
  ...
  docPreset.width = 900;
   docPreset.height = 900;
    ...
    doc.defaultStrokeColor = doc.swatches.getByName("[None]").color;
   
    };
function tualoCode(doc) {
    ...
    var mmatrix = function(rows,cols,fill){
        this.cols=cols;
        this.rows=rows;
        this.data = '';
    ...
    mmatrix.prototype.assigned=function(row,col){
    ...
    Datamatrix.prototype.mappingRows = [
                            8, 10, 12, 14, 16, 18, 20, 22, 24,  // Number of rows for the mapping mmatrix
                            28, 32, 36, 40, 44, 48, 56, 64, 72, 80, 88, 96, 108, 120, 132,
                            6, 6, 10, 10, 14, 14];
    Datamatrix.prototype.mappingCols = [
                            8, 10, 12, 14, 16, 18, 20, 22, 24,  // Number of columns for the mapping mmatrix
                            28, 32, 36, 40, 44, 48, 56, 64, 72, 80, 88, 96, 108, 120, 132,
                            16, 28, 24, 32, 32, 44];
    ...
    Datamatrix.prototype.next=function(etape, totalRows, totalCols, codeWordsBits, datamatrix){ // Place codewords into the mmatrix
    ...
    Datamatrix.prototype.patternShapeStandard=function(datamatrix, bits, row, col, totalRows, totalCols){ // Place bits in the mmatrix (standard or special case)
    ...
    Datamatrix.prototype.placeBitInDatamatrix=function(datamatrix, bit, row, col, totalRows, totalCols){ // Put a bit into the mmatrix
        if (row < 0) {
            row += totalRows;
            col += 4 - ((totalRows+4)%8);
        }
    ...
    Datamatrix.prototype.addFinderPattern=function(datamatrix, rowsRegion, colsRegion, rowsRegionCW, colsRegionCW){ // Add the finder pattern
        ...
        var i=0;
        var j=0;
        var datamatrixTemp = new mmatrix(totalRowsCW+2,totalColsCW+2,0);
    ...
    Datamatrix.prototype.getDigit=function(text, rectangular){
        ...
        var datamatrix = new mmatrix(rowsLengthMatrice,colsLengthMatrice);
        ...
        // Put the codewords into the mmatrix
    ...
    var textarray = ascii.split(/\r\n|\r|\n/)
    var arrayLength = textarray.length
    size = SizeValue.text/(arrayLength-2)*PTS_MM;

        //отступ верхняя строка  
          for (var z=0; z<Margin;z++){
            for (var x=0; x<(textLength+(Margin-1)*2);x++){   
    makeWhiteRect((z)*-size,(x)*size,size,size,doc);      
            }      
          }
    for (var i=1; i<arrayLength-1; i++){
        var partOfText = textarray[i]
        var textLength = partOfText.length

        
        //отступ левый ряд
        for (var y=0; y<Margin;y++){
    makeWhiteRect((i-1+Margin)*-size,(y)*size,size,size,doc);       
           }
       
        for (var x=1; x<textLength-1;x++){
            
            if (partOfText[x]==0){
                makeWhiteRect((i-1+Margin)*-size,(x-1+Margin)*size,size,size,doc);
                }
            else {
                makeBlackRect((i-1+Margin)*-size,(x-1+Margin)*size,size,size,doc);
            }
        }
    //отступ правый ряд
        for (var y=0; y<Margin;y++){
    makeWhiteRect((i-1+Margin)*-size,(x+y+Margin-1)*size,size,size,doc);       
           }
       }
    //отступ нижний ряд
       for (var z=0; z<Margin;z++){
            for (var x=0; x<(textLength+(Margin-1)*2);x++){   
                makeWhiteRect((z+i+Margin-1)*-size,(x)*size,size,size,doc);      
            }
          }
...
function makeBlackRect(a,b,c,d,doc) {
    var blackFill = new CMYKColor();
    blackFill.cyan = 0;
    blackFill.magenta = 0;
    blackFill.yellow = 0;
    blackFill.black = 100;
    var rectRef = doc.pathItems.rectangle(a,b,c,d);
    rectRef.fillColor = blackFill;
};

function makeWhiteRect(a,b,c,d,doc) {
    var whiteFill = new CMYKColor();
    whiteFill.cyan = 0;
    whiteFill.magenta = 0;
    whiteFill.yellow = 0;
    whiteFill.black = 0;
    var rectRef = doc.pathItems.rectangle(a,b,c,d);
    rectRef.fillColor = whiteFill;
};
